...
object ApiClient {
    private const val TAG = "ZomatoApiClient"
...
//    }
    ...
    fun getOrderSummary(context: Context, orderId: String, accessToken: String): OrderDetails? {
        FileLogger.log(context, TAG, "Fetching Order Summary for ID: $orderId")
        try {
            val url = HttpUrl.Builder()
                .scheme("https")
                .host("api.zomato.com")
            ...
            if (response.isSuccessful && responseBody != null) {
                FileLogger.log(context, TAG, "OrderSummary Success: ${response.code}")
                return parseOrderDetails(context, orderId, responseBody)
            }
            FileLogger.log(context, TAG, "OrderSummary Failed. Code: ${response.code}")
            return null
        } catch (e: Exception) {
            FileLogger.log(context, TAG, "OrderSummary Exception", e)
            return null
        }
    }
    ...
    private fun parseOrderDetails(context: Context, orderId: String, jsonString: String): OrderDetails? {
        try {
            ...
            FileLogger.log(context, TAG, "OrderDetails parsed: restaurant=$restaurantName, image=${restaurantImageUrl != null}, cart=$cartTotal, paid=$paidAmount, items=${orderItems.size}")
            return result
        } catch (e: Exception) {
            FileLogger.log(context, TAG, "OrderDetails parse error: ${e.message}", e)
            return null
        }
    }
